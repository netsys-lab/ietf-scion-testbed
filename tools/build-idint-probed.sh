...
# is cgo-only. Same GLIBC_2.36 ceiling guard as build-idint-traceroute.sh
# (containers are Debian 12).
set -euo pipefail
...
if [ "$(printf '%s\nGLIBC_2.36\n' "$max_glibc" | sort -V | tail -1)" != "GLIBC_2.36" ]; then
    echo "error: binary needs $max_glibc > GLIBC_2.36 ceiling" >&2; exit 1
fi

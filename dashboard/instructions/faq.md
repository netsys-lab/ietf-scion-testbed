...
1. **What is this?** A 12-AS SCION network built for IETF 126, running as real
...
2. **What's a "SCION link"?** Each line on the dashboard map is an inter-AS
...
3. **What's the "BGP" badge / the `as150.scion` names?** The same inter-AS
...
5. **How big a payload can I send through my WireGuard tunnel?** Keep
...
7. **Can I just download prebuilt SCION binaries?** Yes — on Ubuntu 24.04
(amd64) the bootstrap script in `laptop.md` sets up an apt repository
(`https://lcschulz.de/scion/apt`) with prebuilt SCION daemon/tools and
scitra packages, and configures everything for you. What you should *not*
use are the official scionproto release binaries: `v0.15.0` ships built
`CGO_ENABLED=0` and its `scion-daemon` panics on startup
(`go-sqlite3 … is a stub`) before it can open a socket. Building from
source (e.g. for arm64) needs `CGO_ENABLED=1` — see the appendix in
`laptop.md`.
